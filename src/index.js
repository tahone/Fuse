...
class Fuse {
    ...
    _prepareSearchers(pattern = '') {
        ...
        if (this.options.tokenize) {
            ...
            const tokens = _filter(pattern.split(this.options.tokenSeparator), (token) => {
                return token !== '';
            });
    ...
    _analyze({key, value, record, index}, {tokenSearchers = [], fullSearcher = [], resultMap = {}, results = []}) {
        ...
        if (typeof value === 'string') {
            ...
            if (this.options.tokenize) {
                ...
                this._log('Token score average:', averageScore, scores)
            }
            ...
            // Always add results because no match is important information for scoring
            if (existingResult) {
